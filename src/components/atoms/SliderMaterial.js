...
export const SliderMaterial = ({ valueSlider, handleChangeSlider, valueSearch, ids }) => {
  useEffect(() => {
    ...
    <div>
      {Number(ids) === LINES.books ? (
        <div className={'div_slider'}>
          <text className={'text_slider'}>1700 год</text>
          <PrettoSlider
            // className={cla}
            value={valueSlider}
          ...
          />
          <text className={'text_slider'}>2021 год</text>
        </div>
      ) : null}
    </div>
  )
